...
# Modified by Hanyang Univ., j-pong.
#
...
class W2lDecoder(object):
    def __init__(self, nbest, tgt_dict):
        ...
        # TODO(dh): what is asg_transitions?
        self.asg_transitions = None
        self.blank = tgt_dict.index('<blank>')
        self.silence = tgt_dict.index('<space>')
        
    def generate(self, x, **unused):
        """Generate a batch of inferences."""
        
        x = x.float().cpu().contiguous()
        return self.decode(emissions=x)
...
class W2lViterbiDecoder(W2lDecoder):
    ...
    def decode(self, emissions):
        ...
        logging.info("decoder input length: " + str(emissions.shape[1]))
        ...
        if self.token_list is not None:
            logging.info(
                "best hypo: "
                + "".join([self.token_list[x] for x in returns[0][0]["tokens"]])
                + "\n"
            )
