...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 9 ] && [ ${stop_stage} -ge 9 ]; then
        ...
        # shellcheck disable=SC2086
        ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
            ${python} -m hynet.bin.asr_train_multi_text \
                --collect_stats true \
                --use_preprocessor true \
                --bpemodel "${bpemodel}" \
                --token_type "${token_type}" \
                ...
                --train_data_path_and_name_and_type "${_asr_train_dir}/text_aux,text_aux,text" \
                ...
                --train_pseudo_data_path_and_name_and_type "${_asr_train_pseudo_dir}/text_aux,text_aux,text" \
        ...
        <"${asr_stats_dir}/train/text_aux_shape" \
            awk -v N="$(<${token_list} wc -l)" '{ print $0 "," N }' \
            >"${asr_stats_dir}/train/text_aux_shape.${token_type}"

        <"${asr_stats_dir}/train_pseudo/text_aux_shape" \
            awk -v N="$(<${token_list} wc -l)" '{ print $0 "," N }' \
            >"${asr_stats_dir}/train_pseudo/text_aux_shape.${token_type}"
    ...
    if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ]; then
        ...
        _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/text_aux,text_aux,text "
        _opts+="--train_shape_file ${asr_stats_dir}/train/speech_shape "
        _opts+="--train_shape_file ${asr_stats_dir}/train/text_shape.${token_type} "
        _opts+="--train_shape_file ${asr_stats_dir}/train/text_aux_shape.${token_type} "
        ...
        _opts+="--train_pseudo_data_path_and_name_and_type ${_asr_train_pseudo_dir}/text_aux,text_aux,text "
        ...
        _opts+="--train_pseudo_shape_file ${asr_stats_dir}/train_pseudo/text_aux_shape.${token_type} "
        ...
        ${python} -m espnet2.bin.launch \
            ...
            --multiprocessing_distributed true -- \
            CUDA_VISIBLE_DEVICES=${ngpu_ids} ${python} -m hynet.bin.asr_train_multi_text \
                --use_preprocessor true \
                --bpemodel "${bpemodel}" \
                --token_type "${token_type}" \
                --token_list "${token_list}" \
